...
* **실시간 통제**: 백그라운드 분석기(Scanner)나 비동기 작업(Runtime)은 시간이 걸립니다. 하지만 API 호출 시점의 차단은 **10ms 미만**으로 결정되어야 서비스 사용성을 해치지 않습니다. Rust 기반의 Axum을 게이트웨이 언어로 선택한 이유도 고성능 실시간 처리를 지향하기 때문입니다.
