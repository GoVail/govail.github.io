...
hero:
  name: "GoVail"
  text: "Enterprise AI 거버넌스 플랫폼"
  tagline: "Rust Gateway · DLP 정책 엔진 · 인텔리전트 라우팅 · 실시간 감사 로그 — 5개 마이크로서비스, 90+ 테스트, 8개 비동기 워크플로우"
  actions:
    - theme: brand
      text: 🏗️ 아키텍처 보기
...
::: info 9. 마이크로서비스 운영기
**[5개 서비스를 한 사람이 관리하는 법](/posts/operational-lessons)**

Docker Compose 기반 로컬 개발 환경, 멀티레포 관리, JSON Schema 계약 전략 — 1인 개발자의 마이크로서비스 운영 회고.
:::

::: info 10. Temporal 워크플로우
**[Temporal로 구현한 Human-in-the-loop 비동기 파이프라인](/posts/temporal-workflows)**

8개 워크플로우, Signal 패턴으로 승인 대기, Runner Control Protocol — Celery 대신 Temporal을 선택한 이유.
:::

::: info 11. 관측 스택
**[Prometheus + Grafana + Loki로 구축한 실시간 관측](/posts/observability-stack)**

상용 APM 대신 오픈소스 스택을 선택한 이유, 대시보드 설계, 로그 중앙화 파이프라인.
:::

## Quick Start {#quick-start}

```bash
# GoVail 전체 스택을 한 번에 실행
git clone https://github.com/GoVail/govail-examples.git
cd govail-examples
cp .env.example .env
docker-compose up -d
...
Gateway · Router · Scanner · SurrealDB · Temporal · Prometheus · Grafana · Loki까지 한 번에 기동됩니다.

## 숫자로 보는 GoVail

| 지표 | 수치 |
|------|------|
| 마이크로서비스 | **5개** (Gateway, Router, Runtime, Scanner, Memory) |
| 언어 | **Rust** (Gateway) + **Python** (4개 서비스) + **TypeScript** (클라이언트) |
| 자동화 테스트 | **90+개** (Runtime 50+ / Router 19 / Scanner 18 / Memory 3) |
| 비동기 워크플로우 | **8개** (Temporal 기반: code_scan, code_review, PR 생성 등) |
| 서비스 간 계약 | **8개** JSON Schema (Runner Control Protocol, Audit Event 등) |
| 옵저버빌리티 | Prometheus + Grafana + Loki + Promtail **풀스택** |
| 클라이언트 | Chrome 확장 · Electron 데스크톱 · TypeScript SDK · Python CLI |

## vs Alternatives

| 기능 | **GoVail** | Portkey | LiteLLM Proxy | Guardrails AI |
|------|:----------:|:-------:|:-------------:|:-------------:|
| DLP 정책 엔진 | ✅ 내장 | ❌ | ❌ | ✅ |
| 온프레미스 배포 | ✅ 완전 자체 호스팅 | ⚠️ 제한적 | ✅ | ✅ |
| Rust 성능 Gateway | ✅ Axum 기반 | ❌ Node.js | ❌ Python | ❌ Python |
| 비동기 워크플로우 | ✅ Temporal 기반 | ❌ | ❌ | ❌ |
| RAG 통합 | ✅ Memory 서비스 | ❌ | ❌ | ❌ |
| 코드 스캐닝 | ✅ Scanner 연동 | ❌ | ❌ | ❌ |
| 감사 로그 | ✅ JSON Schema 기반 | ✅ | ⚠️ 기본 | ⚠️ 기본 |
| 오픈소스 | ✅ Apache 2.0 | ⚠️ 부분 | ✅ | ✅ |
